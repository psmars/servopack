...
def block_draw(b,block0,centre0,color,is_first):
    ...
    if (is_first):
        cv2.fillPoly(frame,[b],(220,220,220))
        cv2.polylines(frame,[b],True,(100,100,100),2)
    else:
        cv2.fillPoly(frame,[b],color)
    ...
    if ret==True:
        ...
        if len(corners)>0:
            ...
            for (m_corner, m_id) in zip(corners, ids):
                ...
                dy = (tr[1]+br[1])/2
                if (first):
                    cv2.circle(frame,(int(cx),int(cy)),4,(0,0,0),-1)
                else:
                    cv2.circle(frame,(int(cx),int(cy)),4,(0,150,0),-1)
            ...
            if (first):
                ...
                first=False
        try:
            img = imutils.resize(frame,width=w)
